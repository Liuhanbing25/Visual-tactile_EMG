...
class CsvLogger:
    ...
    def start(self, emg_path: str, imu_path: str, meta_lines: Optional[List[str]] = None) -> None:
        ...
        # EMG columns note
        self.emg_f.write(
            "# EMG column notes: EMG_t_s=seconds since this Start; sample_idx=device sample index (ADS frame index); fs=sample rate (SPS); gain=gain; flags=0x01(TEST) 0x02(SHORT) 0x04(RLD); emg_valid=1 valid / 0 invalid (spike-guard decision); ch1_raw_uV=raw uV (after spike guard); ch1_filt_uV=filtered uV; ch1_env_uV=envelope (RMS) uV\n")

        # headers
        self.emg_f.write(",".join([
            "EMG_t_s", "sample_idx", "fs", "gain", "flags",
            "emg_valid",
    ...
    def write_emg_block(self,
        ...
        CSV列：
          EMG_t_s,sample_idx,fs,gain,flags,emg_valid,
          ch1_raw_uV,ch1_filt_uV,ch1_env_uV,
          ch2_raw_uV,ch2_filt_uV,ch2_env_uV

        说明：
          - single-channel device：CH2 三列leave blank
          - blank_invalid=True 且 emg_valid=0：CH1/CH2 三列都leave blank（easier to skip in post-processing）
        """
...
class VideoRecorder(QtCore.QThread):
    def __init__(self, camera_idx=0, save_path="tactile_video.mp4"):
        super().__init__()
        self.camera_idx = camera_idx
        self.save_path = save_path
        self.log_path = save_path.replace(".mp4", "_timestamps.csv")
        self.running = False
    ...
    def run(self):
        ...
        with open(self.log_path, mode='w', newline='') as f:
            ...
            while self.running:
                ret, frame = cap.read()
                if ret:
                    # 获取当前高精度系统时间 (用于对齐 EMG)
                    current_time = time.time() 
                    hms = time.strftime("%H:%M:%S", time.localtime(current_time))
                    ms = int((current_time % 1) * 1000)
                    time_str = f"{hms}.{ms:03d}"

                    # 1. 在画面边缘叠加时间戳 (可视化参考)
                    # 注意：为了不干扰 CDU2.py 的圆圈检测，建议将文字放在极角落
                    cv2.putText(frame, time_str, (10, height - 20), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

                    # 2. 写入视频帧
                    out.write(frame)

                    # 3. 记录精确时间戳到 CSV (程序对齐用)
                    log_writer.writerow([frame_count, current_time, time_str])
...
class DeviceWidget(QtWidgets.QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.ck_autoy.toggled.connect(lambda _v: self._reset_auto_y())
...
class OverviewWidget(QtWidgets.QWidget):
    ...
    def _record_stop(self) -> None:
        ...
        if hasattr(self, 'video_thread') and self.video_thread.isRunning():
            ...
            if self.devices: self.devices[0]._log("[VIDEO] Stopped")
